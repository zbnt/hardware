...
module pattern_dram #(parameter axi_width = 32)
(
	input logic clk,

	input logic [7-$clog2(axi_width/8):0] a,
	input logic [axi_width-1:0] d,
	output logic [axi_width-1:0] spo,
	input logic [(axi_width/8)-1:0] we,

	input logic [7-$clog2(axi_width/8):0] dpra,
	output logic [axi_width-1:0] dpo
);
	xpm_memory_dpdistram
	#(
		.ADDR_WIDTH_A(8 - $clog2(axi_width/8)),
		.ADDR_WIDTH_B(8 - $clog2(axi_width/8)),
